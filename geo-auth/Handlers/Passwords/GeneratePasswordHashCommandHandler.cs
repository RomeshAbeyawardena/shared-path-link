...
internal class GeneratePasswordHashCommandHandler(IOptions<PasswordConfiguration> passwordConfigurationOptions,
    ILogger<GeneratePasswordHashCommandHandler> logger) 
    : IRequestHandler<GeneratePasswordHashCommand, PasswordHashResult>
{
    ...
    public Task<PasswordHashResult> Handle(GeneratePasswordHashCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogError(ex, "Unable to generate password hash");
            return Task.FromResult(new PasswordHashResult(null, ex));
        }
    }
}
